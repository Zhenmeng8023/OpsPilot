...
| --- | --- | --- | --- | --- |
| 2026-05-09 | `v1.0.0` release closure | Closed the V1.0 release gate. | Aligned the root version marker to `1.0.0`, confirmed remote GitHub Actions success on `master` and `v1.0.0-beta.1`, and added the V1.1 design baseline. | Synced README, CHANGELOG, V1.0 release evidence, V1.0 design docs, and V1.1 planning docs; local validation remains green. |
| 2026-05-09 | V1.0 in progress / pending | Squashed the active V1.0 SQL migration chain. | Replaced root `000005`-`000015` with `000005_v10_productization_bundle`, archived the original incremental files, and aligned the root version marker to `1.0.0`. | Validated MySQL apply, rollback, and schema equivalence against the legacy chain; synced README, CHANGELOG, V1.0 release evidence, design docs, UML notes, and migration archive notes. |
| 2026-05-09 | V1.0 in progress / pending | Closed another frontend i18n cleanup slice. | Localized the Workflow default sample definition and UI action reasons, localized Audit export failure copy plus actor type/result rendering, and completed the related zh/en dictionary entries used by Agent/Webhook/Audit/Workflow pages. | Synced global CHANGELOG, README, V1.0 release evidence, and version design docs; validated `npm run build` and `git diff --check`. |
| 2026-05-08 | V1.0 in progress / pending | Added Alert grouping operations view. | Added `GET /api/v1/alert-groups`, grouped alert aggregation by rule/host group/severity/fingerprint, Metrics page alert-group operations table, i18n copy, OpenAPI schema, and alert grouping unit/handler tests. | Synced README, CHANGELOG, V1.0 release evidence, version design docs, OpenAPI, and UML notes; validated Go test/vet, web build, migration pair check, OpenAPI/router check, and git diff check. |
| 2026-05-08 | V1.0 in progress / pending | Added Agent Host Group batch operations and group-scoped maintenance windows. | Added `host_group_id` maintenance scope migration, Host Group batch-disable and diagnostics APIs, Agent page group operations, alert/schedule maintenance matching by group, and README/CHANGELOG status cleanup. | Synced V1.0 release evidence, version design docs, OpenAPI, and UML notes; validated Go test/vet, web build, migration pair check, OpenAPI/router check. |
