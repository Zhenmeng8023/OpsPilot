...
| --- | --- | --- | --- |
| 2026-05-09 | V1.0 in progress / pending | Cleared the remaining GitHub Actions runtime deprecation risk. | CI now sets `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`, keeping the existing workflow matrix and forcing JavaScript actions off the deprecated Node 20 runtime. |
...
## Remaining Risks Before V1.0.0

- Observe one green GitHub Actions run after these CI changes are pushed. Local equivalents now pass, but the final CI state is still a remote-system fact.
- Retain desktop and narrow-screen screenshot evidence for the final release report, especially for Dashboard, Workflow, Notifications, and Audit pages.
- Workflow retry/timeout, metrics rollup/retention, and alert grouping paths now have working baseline coverage, but broader database integration depth is still recommended as post-V1.0 hardening rather than a release blocker.
