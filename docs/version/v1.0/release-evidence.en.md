...
| --- | --- | --- | --- |
| 2026-05-10 | V1.0 hardening / pending | Closed remaining Go vulnerability risk and removed the temporary allowlist bypass. | The Go toolchain baseline is now `1.26.3` (`server/go.mod`), and `scripts/govulncheck.ps1` defaults to an empty allowlist with strict failure behavior restored; also upgraded `github.com/jackc/pgx/v5` to `v5.9.0` and `filippo.io/edwards25519` to `v1.1.1`; validated on `go1.26.3` with passing `go test ./...`, `go vet ./...`, and `govulncheck -show verbose ./...` showing 0 vulnerabilities. |
| 2026-05-10 | V1.0 hardening / pending | Fixed Frontend CI install failures caused by lockfile drift under strict npm. | `web/package-lock.json` now includes transitive `@emnapi/core` / `@emnapi/runtime` / `@napi-rs/wasm-runtime` entries and normalizes tarball URLs to npmjs; `web/.npmrc` now pins the official registry and enables `replace-registry-host=always`; CI install steps now pass `--registry=https://registry.npmjs.org`; strict `npm@11.11.0` `npm ci --include=optional` is green locally. |
