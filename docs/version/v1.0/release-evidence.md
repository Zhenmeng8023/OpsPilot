...
| 日期 | 版本/提交 | 更迭内容 | 同步说明 |
| 2026-05-10 | V1.0 加固中 / 待提交 | 收口 Go 漏洞残余风险，移除临时放行。 | Go toolchain 基线升级到 `1.26.3`（`server/go.mod`），`scripts/govulncheck.ps1` 默认 allowlist 清空并恢复严格失败策略；同时升级 `github.com/jackc/pgx/v5` 到 `v5.9.0`、`filippo.io/edwards25519` 到 `v1.1.1`，在 `go1.26.3` 下复验 `go test ./...`、`go vet ./...`、`govulncheck -show verbose ./...` 全部通过且 0 漏洞。 |
